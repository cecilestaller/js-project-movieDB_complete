...
const movieOutput = document.body.querySelector('#movie-output');
...
showValues(movies); // --> da Filmausgabe in function gespeichert mit "neutralem" Parameter (array), kann sie für alle array kopien/ sort functions für die ausgabe der sortierten filme wiederverwendet werden



// STEP 2: search movie

const searchInput = document.body.querySelector('#movie-input');
...
const jahrHoch = () => {
    movieOutput.innerHTML = " ";
    const arrayKopie = [...movies]; // geht mit spread operator und dann direkt sort an die arraykopie ODER sort an original array währen in neuer variablen gespeichert...
    arrayKopie.sort((yearA, yearB) => yearA[1] - yearB[1]);
    showValues(arrayKopie); // da ich die film Ausgabe in Function gespeichert habe mit Parameter (array), kann ich sie für die kopierten und Sortierten arrays wiederverwenden.
};

// STEP 4: Sort year down

const jahrRunter = () => {
    movieOutput.innerHTML = " ";
    const arrayKopie = [...movies]; 
    arrayKopie.sort((yearA, yearB) => yearB[1] - yearA[1]);
    showValues(arrayKopie); 
};


// STEP 5: sortierung nach Ranking
